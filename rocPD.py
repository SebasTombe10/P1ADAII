...
def asignar_materias(estudiantes: dict, cupos: dict) -> tuple:
    ...
    for asignacion in asignaciones:
        ...
        asignacion_subproblema, insatisfaccion_actual = asignar_materias(
            aux_estudiantes, cupos_restantes
        )

        if len(estudiantes) == 1:
            insatisfaccion_general = (
                insatisfaccion(estudiantes, asignacion) + (insatisfaccion_actual)
            ) / len(estudiantes)
        else:
            insatisfaccion_general = (
                insatisfaccion(estudiantes, asignacion)
                + (insatisfaccion_actual * (len(estudiantes) - 1))
            ) / len(estudiantes)
